...
year_selector = st.number_input(label='Model Year', min_value=1997, max_value=2024)
...
with col5:
    try:
        drivetrains = pd.DataFrame(conn.sql(f'''SELECT DISTINCT drivetrain FROM listings WHERE TRIM(make) = '{str(make_selector)}' AND TRIM(model) = '{str(model_selector)}' AND TRIM(trim) LIKE '%{str(trim_selector)}%' AND TRIM(transmission) LIKE '%{str(transmission_selector)}%' AND year = {year_selector} ORDER BY trim''').to_df())
...
with col6:
    try:
        exterior_colors = pd.DataFrame(conn.sql(f'''SELECT DISTINCT exterior_color FROM listings WHERE TRIM(make) = '{str(make_selector)}' AND TRIM(model) = '{str(model_selector)}' AND TRIM(trim) LIKE '%{str(trim_selector)}%' AND year = {year_selector} ORDER BY exterior_color''').to_df())
...
with col7:
    try:
        ...
        interior_colors = pd.DataFrame(conn.sql(f'SELECT DISTINCT interior_color FROM listings ORDER BY exterior_color').to_df())
        interior_colors = pd.Series(interior_colors['interior_color'])
        interior_colors = interior_colors.tolist()
        int_color_selector = st.selectbox(label='Interior Color', options=[int_color for int_color in interior_colors])
st.text("")
